import { app, BrowserWindow, ipcMain, systemPreferences, Tray, Menu, nativeImage, NativeImage, dialog, shell } from 'electron';
...
ipcMain.handle(
    ...
    async (
        ...
        // Check for Keys
        const storeGeminiKey = store.get('geminiApiKey');
        const envGeminiKey = process.env.MEMO_GEMINI_API_KEY;
        const geminiKey = storeGeminiKey || envGeminiKey;

        const openAIKey = process.env.MEMO_OPENAI_API_KEY || process.env.OPENAI_API_KEY;

        if (!openAIKey && !geminiKey) {
            return { success: false, error: 'No API key configured (OpenAI or Gemini).' };
        }
        ...
        try {
            ...
            if (geminiKey) {
                ...
                // Use configured model or fallback to specific version
                const storeModel = store.get('geminiModel');
                const envModel = process.env.MEMO_GEMINI_MODEL;
                const geminiModel = storeModel || envModel || 'gemini-2.0-flash';
                console.log(`🤖 Using Gemini model: ${geminiModel}`);
                const geminiUrl = `https://generativelanguage.googleapis.com/v1beta/models/${geminiModel}:generateContent?key=${geminiKey}`;

                // Gemini performs better when system instructions are part of the prompt in the REST API
                const fullGeminiPrompt = `${systemInstruction}\n\n${userPrompt}`;
                ...
                if (candidate?.content?.parts?.[0]?.text) {
                    return { success: true, summary: candidate.content.parts[0].text.trim() };
                } else if (candidate?.finishReason) {
                    return { success: false, error: `Gemini blocked response: ${candidate.finishReason}` };
                }
...
ipcMain.handle('save-settings', (_event, settings: StoreType) => {
    if (settings.geminiApiKey !== undefined) store.set('geminiApiKey', settings.geminiApiKey);
    if (settings.assemblyAiKey !== undefined) store.set('assemblyAiKey', settings.assemblyAiKey);
    if (settings.geminiModel !== undefined) store.set('geminiModel', settings.geminiModel);
    return { success: true };
});
...
ipcMain.handle('start-recording', async () => {
    ...
    if (recordingWidgetWindow) {
        // Pass AssemblyAI key from store if available
        const assemblyKey = store.get('assemblyAiKey') || process.env.VITE_ASSEMBLY_AI_KEY;
        recordingWidgetWindow.webContents.send('recording-state-changed', {
            isRecording: true,
