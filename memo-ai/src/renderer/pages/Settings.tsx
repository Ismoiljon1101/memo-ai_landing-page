...
export default function Settings() {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        const apiSettings = await window.electronAPI.getSettings();
  ...
  const SettingToggle = ({
    ...
    <div className="flex items-center justify-between rounded-lg border border-slate-200 bg-white/70 px-3 py-2.5">
      ...
      <button
        onClick={onChange}
        className={`relative inline-flex h-6 w-10 shrink-0 rounded-full border border-transparent transition-colors ${value ? 'bg-slate-900 text-white' : 'bg-slate-200'
          }`}
        type="button"
      >
        <span
          className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${value ? 'translate-x-4' : 'translate-x-0.5'
            }`}
        />
      </button>
    </div>
      ...
      <div className="mx-auto w-full max-w-3xl space-y-5">
        ...
        <Card className="border-slate-200">
          <CardHeader className="pb-3">
            ...
            <CardDescription>Enter your own API keys for testing and development.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-slate-500">Used for meeting summarization.</p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-700">Gemini Model</label>
              <Input
                ...
                className="border-slate-200"
              />
              <p className="text-xs text-slate-500">Override the default model (e.g. gemini-1.5-pro).</p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-700">AssemblyAI API Key</label>
              <Input
                type="password"
                placeholder="Paste your key here..."
                value={settings.assemblyAiKey || ''}
                onChange={(e) => handleChange('assemblyAiKey', e.target.value)}
                className="border-slate-200"
              />
              <p className="text-xs text-slate-500">Used for real-time transcription.</p>
            </div>
          </CardContent>
        </Card>
