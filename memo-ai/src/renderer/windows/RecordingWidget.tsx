...
export default function RecordingWidget() {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (settings.assemblyAiKey) {
          setUserApiKey(settings.assemblyAiKey);
        }
      } catch (e) {
  ...
  const effectiveApiKey = userApiKey || ASSEMBLY_AI_KEY;
  ...
  const handleStart = async () => {
    try {
      if (!effectiveApiKey) {
        alert('AssemblyAI API key is not configured. Add it in Settings or via VITE_ASSEMBLY_AI_KEY.');
        return;
      }
      clearAITranscript();
      clearAIError();
      ...
      <div className="flex items-center justify-between gap-4 p-3 border-b border-gray-200 bg-gray-100">
        <div className="flex gap-2">
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-md font-medium text-xs transition cursor-pointer ${activeTab === 'notes'
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-md font-medium text-xs transition cursor-pointer ${activeTab === 'transcript'
              ? 'bg-white text-black border border-gray-300'
              : 'text-gray-600 hover:text-black hover:bg-gray-100 border border-transparent'
              }`}
            style={{ WebkitAppRegion: 'no-drag' } as any}
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          className={`inline-flex items-center justify-center rounded-md border px-3 py-1.5 text-xs font-medium transition ${created
