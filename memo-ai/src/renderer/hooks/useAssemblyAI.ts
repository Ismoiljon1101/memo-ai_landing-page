...
export function useAssemblyAI(options: UseAssemblyAIOptions) {
      ...
      navigator.mediaDevices.getUserMedia({ audio: true })
        .then((stream) => {
          ...
          wsRef.current.onerror = (event) => {
            ...
            let errorMsg = 'Connection to AssemblyAI failed.';

            // Try to infer clearer message
            if (event instanceof ErrorEvent && event.message) {
              errorMsg += ` ${event.message}`;
            } else {
              // Common causes for silent failure often related to auth
              errorMsg += ' Check your internet connection and VITE_ASSEMBLY_AI_KEY.';
            }
  ...
  const connectWithTimeout = useCallback(async () => {
    ...
    const timeoutPromise = new Promise<void>((_, reject) => {
      setTimeout(() => reject(new Error('Connection timed out. Check your network or API key.')), timeoutMs);
    });
